...
cd $BUDGETS_COMPARATOR_GENCAT; FAST_RUN=$FAST_RUN bin/rails runner $COMPARATOR_GENCAT_ETL/operations/gobierto-budgets/budgets_forecast/update.rb 2016-12-31 $STORAGE_DIR

# Extract/Load > Update budgets execution since 31/12/2016
cd $BUDGETS_COMPARATOR_GENCAT; bin/rails runner $COMPARATOR_GENCAT_ETL/operations/gobierto-budgets/budgets_execution/update.rb 2016-12-31 $STORAGE_DIR

# Transform/Load > Update total budgets for 2016-2018 forecast data
cd $GOBIERTO_ETL_UTILS/operations/gobierto_budgets/update_total_budget/; ./run.rb '2016 2017 2018' $STORAGE_DIR/scanned_organizations_ids.update.txt

# Transform/Load > Update total budgets for 2016-2018 execution data
cd $GOBIERTO_ETL_UTILS/operations/gobierto_budgets/update_total_budget; ./run.rb '2016 2017 2018' $STORAGE_DIR/scanned_organizations_ids.update_execution.txt
